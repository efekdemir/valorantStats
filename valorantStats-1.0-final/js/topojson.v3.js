...
(function (global, factory) {
  ...
  var hashset = function (size, hash, equal, type, empty) {
    ...
    var store = new type(
        (size = 1 << Math.max(4, Math.ceil(Math.log(size) / Math.LN2)))
      ),
      mask = size - 1;
  ...
  var join = function (topology) {
    ...
    function sequence(i, previousIndex, currentIndex, nextIndex) {
      ...
      if (leftIndex >= 0) {
        ...
        if (
          (leftIndex !== previousIndex || rightIndex !== nextIndex) &&
    ...
    function index() {
      var indexByPoint = hashmap(
          coordinates.length * 1.4,
          hashIndex,
          equalIndex,
          Int32Array,
          -1,
  ...
  var dedup = function (topology) {
    ...
    function dedupRing(arc) {
      ...
      if (
        (endArcs = arcsByEnd.get(
          (endPoint = coordinates[arc[0] + findMinimumOffset(arc)])
        ))
      ) {
        for (i = 0, n = endArcs.length; i < n; ++i) {
          endArc = endArcs[i];
    ...
    function equalRing(arcA, arcB) {
      ...
      for (var i = 0; i < n; ++i) {
        if (
          !equalPoint(
            coordinates[ia + ((i + ka) % n)],
            coordinates[ib + ((i + kb) % n)]
          )
        )
          return false;
      }
  ...
  var topology = function (objects, quantization) {
    var bbox = bounds((objects = geometry(objects))),
      transform =
        quantization > 0 && bbox && prequantize(objects, bbox, quantization),
    ...
    function indexGeometry(geometry$$1) {
      if (geometry$$1 && indexGeometryType.hasOwnProperty(geometry$$1.type))
        indexGeometryType[geometry$$1.type](geometry$$1);
    }
    ...
    function filterGeometry(input) {
      ...
      switch (input.type) {
        ...
        case "GeometryCollection": {
          ...
          output = arcs.length
            ? { type: "GeometryCollection", geometries: arcs }
            : { type: null };
          break;
        }
        default:
  ...
  var filterWeight = function (topology, minWeight, weight) {
    ...
    return function (ring, interior) {
      return (
        weight(
          feature(topology, { type: "Polygon", arcs: [ring] }).geometry
            .coordinates[0],
          interior
        ) >= minWeight
      );
